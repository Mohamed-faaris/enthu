...
async function checkEligibilityHelper(eventId: string, studentIds: string[]) {
  ...
  for (const sid of studentIds) {
    ...
    if (eventRow.gender !== "mixed" && st.gender !== eventRow.gender) {
      eligible = false;
      reasons.push(
        `Student ${st.firstName} ${st.lastName} gender ${st.gender} does not match event gender ${eventRow.gender}`
      );
    }
    if (st.studyingClass < category.minClass || st.studyingClass > category.maxClass) {
      eligible = false;
      reasons.push(
        `Student ${st.firstName} ${st.lastName} class ${st.studyingClass} outside category range ${category.minClass}-${category.maxClass}`
      );
    }
  }
